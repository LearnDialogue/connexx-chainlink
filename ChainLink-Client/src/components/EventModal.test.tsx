...
vi.mock('react-leaflet', () => ({
  ...
  const friendStatusesMock = {
    request: {
      ...
      variables: { currentUsername: 'testUser', usernameList: ['testUser', 'friend1'] },
    },
    result: { data: { getFriendStatuses: [{ otherUser: 'friend1', status: 'friend' }] } },
  };
  ...
  describe('EventModal Component', () => {
    test('renders correctly with event data and routeData', async () => {
      ...
      render(
        <MockedProvider mocks={[routeMock, friendStatusesMock]} addTypename={false}>
          <MemoryRouter>
      ...
      await waitFor(() => {
        ...
        expect(within(createdByElement).getByText('testUser')).toBeInTheDocument();
      });
      await waitFor(() => expect(screen.getByText('~~~Test event description.~~~')).toBeInTheDocument());
    ...
    test('calls setEvent when close button is clicked', async () => {
      ...
      render(
        ...
        await waitFor(() => expect(document.querySelector('.ride-card-close-modal')).toBeInTheDocument());
