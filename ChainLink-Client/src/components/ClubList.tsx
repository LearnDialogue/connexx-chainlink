...
const ClubList: React.FC<ClubListProps> = ({ username }) => {
  ...
  const { loading: thisUserLoading, data: thisUserData} = useQuery(
    FETCH_USER_BY_NAME,
  ...
  const [acceptClubInvitation] = useMutation(JOIN_CLUB, { onCompleted: () => setSelectedClub(null) });
  ...
  const [declineClubInvitation] = useMutation(DECLINE_TO_JOIN, { onCompleted: () => setSelectedClub(null) });
  const handleReject = (clubId: string) => declineClubInvitation({ variables: { clubId, userId: thisUserData?.getUser?.id } });
    ...
    <div className="profile-page-clubs-container">
      ...
      <div className="profile-page-club-list">
        {clubsLoading ? (
          ...
          clubsData.getClubs.map((club: any) => (
            <div
              key={club.id}
